...
def loadDataSet(fileName):
    ...
    with open('/home/qjy/ai/task/gesture/data/3.txt') as fp:
        for line in fp.readlines():
            lineArr = line.strip().split()
            dataMat.append([float(x) for x in lineArr])
    with open('/home/qjy/ai/task/gesture/data/1.txt') as fp:
        for line in fp.readlines():
            lineArr = line.strip().split()
            dataMat.append([float(x) for x in lineArr])
    with open('/home/qjy/ai/task/gesture/data/0.txt') as fp:
        for line in fp.readlines():
            lineArr = line.strip().split()
            dataMat.append([float(x) for x in lineArr])
    with open('/home/qjy/ai/task/gesture/data/5.txt') as fp:
        for line in fp.readlines():
            lineArr = line.strip().split()
            dataMat.append([float(x) for x in lineArr])
    with open('/home/qjy/ai/task/gesture/data/4.txt') as fp:
        for line in fp.readlines():
            lineArr = line.strip().split()
            dataMat.append([float(x) for x in lineArr])
    return dataMat
...
def mainTrain(datanum=50, labelnum=6, k1=1.3):
    ...
    judge_feats = []
    datMat = np.mat(data)
    labels = [1] * datanum + [-1] * datanum * 5
    b, alphas = smoP(data, labels, 20000, 0.0000001, 10000000, ('rbf', k1))
    labelMat = np.mat(labels).transpose()
    svInd = np.nonzero(alphas.A > 0)[0]
    judge = (datMat[svInd], labelMat[svInd], alphas[svInd], b)
    for row in data:
        da = np.mat([row])[0, :]
...
def get_video_guass_hsv(judge_feats, datanum=50, labelnum=3, k1=1.3):
